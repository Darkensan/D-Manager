...
TEMP=/tmp/answer$$
whiptail --fb --clear --title "[D] - Manager" --menu "      Ubuntu 16.04/18.04 Denarius's FS Nodes Manager :" 20 0 0 \
...
whiptail --title "D-Compile" --msgbox "This procedure will prepare the VPS installing and updating all the libraries and dependancies required." 12 78
...
echo -e "${Green} Building some directories to use installing nodes${NC}"
	if [ ! -d /var/lib/masternodes ]
	then
		echo -ne $(mkdir /var/lib/masternodes);
	fi
	if [ ! -d /var/lib/masternodes/variants ]
	then
		echo -ne $(mkdir /var/lib/masternodes/variants);
	fi
	if [ ! -d /etc/masternodes ]
	then
		echo -ne $(mkdir /etc/masternodes);
	fi
echo -e "\n"
...
function trap_ctrlc ()
{
    # perform cleanup here
    echo -e "${Red} Ctrl-C caught...performing clean up${NC}"
	rm -rf /var/lib/masternode/* > /dev/null 2>&1;
	rm -rf /etc/masternodes/* > /dev/null 2>&1;
	rm -rf /usr/local/bin/denariusd > /dev/null 2>&1;
...
# when signal 2 (SIGINT) is received
trap "trap_ctrlc" 2
whiptail --title "D-Compile" --msgbox "This procedure will compile a daemon if not already present, create and populate folders and files for the numbers of nodes to install. Chaindata is downloaded and unzipped into each node folder for faster syncronization. Denarius*X*.conf files will be populated adding 25 random nodes each, aswell adding rpcpassword, rpcport and ip. ( so far only ipv4 for 1 node )" 12 78
clear
echo -e "\n"
...
echo -e "\n"
echo -e "${LGreen}${BK} Thanks for using this script, pls report bugs in D's Discord                ${NC}"
		;;
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
3)
...
ifs=$(ls /etc/masternodes/ | grep 'denarius.*\.conf' | wc -l)
clear
whiptail --title "D-Keys" --msgbox "This procedure will prompt for a FS node private key, then add the string to .conf file(s). Remember to use different PrivKey for each FS nodes installed." 10 78
echo -e "${LGreen}---------------------------------------------------------------------------------${NC}"
while [ $n -lt $ifs ]
do
...
n=0
ifs=$(ls /etc/masternodes/ | grep 'denarius.*\.conf' | wc -l)
whiptail --title "D-Stop" --msgbox "This procedure will send a Stop command to all the installed FS Nodes daemon(s)" 8 78
clear
